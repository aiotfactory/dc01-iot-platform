...
public class ModuleHandlerCamera extends ModuleHandler {
	...
	{
		DataCamera dataCamera = (DataCamera)commModel.getUpload();
		if(dataCamera.getPicId()==null || dataCamera.getPicSize()<1024)
			return null;
		byte[] picData=getMongoDBService().fileReadByte(dataCamera.getPicId());
		if(picData!=null && picData.length<1024)
			return null;
		Object[] result=new Object[3];
		result[0]="image";
		result[1]=picData;
		result[2]=dataCamera.getPicId();
		return result;
	}
    
	public Object getRecentData(DeviceModel deviceModel,int timeSeconds)
	{
    	Date timeBefore =MiscUtil.dateAddSeconds(new Date(), timeSeconds*-1);
    ...
    {        	
        ...
        List<DataCommModel> dataCommModelList=getMongoDBService().find(conditionMap, sort, DataCommModel.class,displayTypeInt*page,displayTypeInt*(page+1),null);
