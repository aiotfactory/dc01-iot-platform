...
public class WebController {
    ...
    {
        String[][] breadcrumbs = {
                {miscUtil.utilLocal("breadcrumb_home"), "dashboard"}
            };
        ...
        model.addAttribute("siteCompany",miscUtil.utilLocal("iot_factory"));
    	...
    	if((id!=null)&&(content!=null)&&(content.length()>0))
    	{
    		 ...
    		 if(captchaModel!=null) {
    			 if(captchaModel.getContent().equalsIgnoreCase(content))
 					...
 					result.put("status", miscUtil.utilLocal("invalid_captcha") );
 					result.put("captchaShow", 1);
    				return -1;
    			 }
    		 }
    	}
		result.put("status",  miscUtil.utilLocal("input_captcha_confirm") );
		result.put("captchaShow", 1);
    	return -2;
    }
    ...
    public String pubSafeServer(@RequestParam Map<String, Object> paramsReq,@RequestBody byte[] configData)
    {
    	...
    	ModuleDefine.MODULE_CONFIG.getHandler().recv(deviceModel, msgReq,null);
    ...
    public ResponseEntity<Map<String,Object>> pubUserForgetRefresh(@RequestBody Map<String, Object> params,HttpServletRequest request, HttpServletResponse response) {
		...
		{
    		result.put("status", miscUtil.utilLocal("wrong_email"));
            return ResponseEntity.ok(result);
		}
    	...
    	switch(step)
    	{
    		case 1://input email
    			...
    			if((userModel.getVerificationCodeReqTime()==null)||(MiscUtil.dateDiff(userModel.getVerificationCodeReqTime(), cur)>10*60))
    			{
	    			...
	    			senderService.emailVerificationCode(email, userModel,userModel.getVerificationCode());
    			}
    			result.put("info", miscUtil.utilLocal("check_email_and_fill"));
    			result.put("status", "ok");
    			break;
    		case 2://input verification
    			if(code.length()<6)
    			{
    	    		result.put("status", miscUtil.utilLocal("verification_code_wrong"));
    	            return ResponseEntity.ok(result);
    			}
    			if(userModel.isVerificationCodeRspResult()==true)
    			{
    	    		result.put("status", miscUtil.utilLocal("repeat_verification"));
    	            return ResponseEntity.ok(result);
    			}
    			if(MiscUtil.dateDiff(userModel.getVerificationCodeReqTime(), cur)>30*60)
    			{
       	    		result.put("status", miscUtil.utilLocal("verification_code_expired"));
    	            return ResponseEntity.ok(result);
    			}
    			if(!code.equals(userModel.getVerificationCode()))
    			{
    	    		result.put("status", miscUtil.utilLocal("verification_code_incorrect"));
    	            return ResponseEntity.ok(result);
    			}
    			...
    			result.put("step", 3);
    			result.put("info", miscUtil.utilLocal("set_new_password") );
    			result.put("status", "ok");
    			break;
    		case 3://input password
    			if((password1.length()==0)||(password2.length()==0)||(!password1.equals(password2))) //check password
    			{
    	    		result.put("status", miscUtil.utilLocal("invalid_password"));
    	            return ResponseEntity.ok(result);
    			}
    			...
    			{
    	    		result.put("status", miscUtil.utilLocal("invalid_info") );
    	            return ResponseEntity.ok(result);
    			}
    			if(MiscUtil.dateDiff(userModel.getVerificationCodeRspTime(), cur)>30*60)//check time
    			{
       	    		result.put("status", miscUtil.utilLocal("time_expired") );
    	            return ResponseEntity.ok(result);
    			}
    			if(MiscUtil.userPasswordMatch(userModel.getPassword(), password1))
    			{
       	    		result.put("status", miscUtil.utilLocal("password_same") );
    	            return ResponseEntity.ok(result);
    			}
    			userModel.setVerificationCodeRspResult(false);
    			userModel.setVerificationCodeRspTime(null);
    	    	...
    	    	miscUtil.logout(response);
    			result.put("step", 4);
    			result.put("info", miscUtil.utilLocal("password_reset_success"));
    			result.put("status", "ok");
    			break;
    	}
        return ResponseEntity.ok(result);
    ...
    public String pubUserActivate(@RequestParam Map<String, Object> paramsReq,HttpServletRequest request,Model model) {
    	...
    	String resultStr=miscUtil.utilLocal("wrong_link");
    	Date now=new Date();
    	...
    	if((userId.length()>0)&&(token.length()>0))
    	{
	        ...
	        {        		
	        	...
	        	if(userModel.isEmailActivationRspResult()||(userModel.getUserStatus()!=UserModel.UserStatus.WAIT_ACTIVATION))
	        		resultStr=miscUtil.utilLocal("no_need_activate_again") ;
	        	else {
	        		resultStr=miscUtil.utilLocal("user_activated");
	        	...
	        	resultStr=miscUtil.utilLocal("wrong_activation") ;
    	}
    	model.addAttribute("siteCompany",miscUtil.utilLocal("iot_factory"));
    ...
    public ResponseEntity<Map<String,Object>> pubLoginRefresh(@RequestBody Map<String, Object> params,HttpServletRequest request, HttpServletResponse response) {
    	...
    	if((matchResult[1]==null)||(((UserModel)matchResult[1]).getUserStatus()==UserModel.UserStatus.WAIT_ACTIVATION))
    	{
			...
			if(userModel!=null)//before email is activated, user can change password and send email to activate again
			{
				...
				if((userModel.getEmailActivationReqTime()==null)||(MiscUtil.dateDiff(userModel.getEmailActivationReqTime(), cur))>120)
				{
					...
					senderService.emailActivation(login,userModel.getEmailActivationToken(),userModel,password);
					result.put("status", miscUtil.utilLocal("activation_link_sent") );
				}else
					result.put("status",miscUtil.utilLocal("submit_too_frequently") );			
		...
		if((matchResult!=null)&&(((Boolean)matchResult[0])==true))
		{
			...
			{
	    		result.put("captchaShow", 1);
				result.put("status", miscUtil.utilLocal("user_disabled") );
			}
			else if(userModel.getUserStatus()==UserModel.UserStatus.FREEZED)
			{
	    		result.put("captchaShow", 1);
				result.put("status", miscUtil.utilLocal("user_freezed") );
			}
			else
				result.put("status", "ok");
    	}else {
    		result.put("captchaShow", 1);
    		result.put("status", miscUtil.utilLocal("user_password_wrong") );
    	}
        return ResponseEntity.ok(result);
    }
	@GetMapping("/pub/doc")
    ...
    private UserModel utilUserCreate(String parentId,String createById,UserModel.UserStatus userStatus,String email,String password,Date cur,String actToken,String ip)
    {
    	...
    	userModel.setAccountBalance(0L);
    	if(actToken!=null)
    	{
    		userModel.setEmailActivationReqTime(cur);
    ...
    {    	
    	if(login!=null)
    	{
    		...
    		{
	    		if(result!=null)
	    			result.put("status", miscUtil.utilLocal("user_name_empty"));
	    		return false;
    		}
    		if(login.contains(","))
    		{
	    		if(result!=null)
	    			result.put("status", miscUtil.utilLocal("user_name_contain_comma") );
	    		return false;
    		}
    	}
    	...
    	{
    		if(result!=null)
    			result.put("status", miscUtil.utilLocal("password_empty") );
    		return false;
    	}
    	if((login!=null)&&(configProperties.LOGIN_VALID()==1)&&(!MiscUtil.emailValid(login)))
    	{
    		if(result!=null)
    			result.put("status", miscUtil.utilLocal("not_valid_email") );
    		return false;
    	}
    	if((password!=null)&&(configProperties.PASSWORD_VALID()==1)&&(!MiscUtil.passwordValid(password)))
    	{
    		if(result!=null)
    			result.put("status", miscUtil.utilLocal("password_rule") );
    		return false;
    	}
    	return true;
    }
    private ApiKeyModel apiKeyValid(String key,String api,Map<String,Object> result)
    {
    	...
    	if(api.length()<3)
    	{
    		result.put("status", miscUtil.utilLocal("api_wrong") );
    		return null;
    	}
    	if((key==null)||(key.length()!=32))
    	{
    		result.put("status", miscUtil.utilLocal("key_is_wrong"));
    		return null;
    	}
    	ApiKeyModel apiKeyModel=mongoDBService.findOneByField("key", key, ApiKeyModel.class);
    	if(apiKeyModel==null)
    	{
    		result.put("status", miscUtil.utilLocal("key_not_exist"));
    		return null;
    	}
    	if((apiKeyModel.getUserStatus()==null)||(apiKeyModel.getUserStatus()!=UserModel.UserStatus.ENABLED))
    	{
    		result.put("status", miscUtil.utilLocal("user_not_enabled") );
    		return null;
    	}
    	//new minute
    	boolean minuteChange=false,dayChange=false;
    	if((apiKeyModel.getLatestCallTime()==null)||((apiKeyModel.getLatestCallTime().getTime()/1000/60)!=(now.getTime()/1000/60)))
    		minuteChange=true;
    	if((apiKeyModel.getLatestCallTime()==null)||((apiKeyModel.getLatestCallTime().getTime()/1000/(3600*24))!=(now.getTime()/1000/(3600*24))))
    		dayChange=true;
    	...
    	{
    		result.put("status", miscUtil.utilLocal("exceed_limit_per_day") );
    		return null;
    	}
    	if((apiKeyModel.getLimitPerMinute()!=null)&&(apiKeyModel.getTimesPerMinute()>=apiKeyModel.getLimitPerMinute()))
    	{
    		result.put("status", miscUtil.utilLocal("exceed_limit_per_minute"));
    		return null;
    	}
    	if((apiKeyModel.getApiLimitMap()!=null)&&(callModel!=null))
    	{
    		...
    		if(limitModel!=null)
    		{
    			...
    			{
    	    		result.put("status",  miscUtil.utilLocal("exceed_limit_per_minute_for") + " "+api);
    	    		return null;
    			}
    			if((limitModel.getLimitPerDay()!=null)&&(callModel.getTimesPerDay()!=null)&&(callModel.getTimesPerDay()>=limitModel.getLimitPerDay()))
    			{
    	    		result.put("status", miscUtil.utilLocal("exceed_limit_per_day_for") + " "+api);
    	    		return null;
    			}
    		}
    	}
    ...
    public ResponseEntity<Map<String,Object>> apiCall(@RequestParam Map<String, Object> paramsReq,@RequestBody(required = false) Map<String, Object> paramsBody,HttpServletRequest request) {
    	...
    	if(userModel.getParentId()!=null) {
    		result.put("status", miscUtil.utilLocal("child_user_not_call_api") );
    		return ResponseEntity.ok(result);
    	}
		...
		if(resultApi==null) {
			result.put("status", miscUtil.utilLocal("unknown_error") );
			return ResponseEntity.ok(result);
		}
    	return resultApi;
    }
	private ResponseEntity<Map<String, Object>> apiCallExe(String api,Map<String, Object> paramsBody,HttpServletRequest request) 
	{
		...
		switch(api)
		{
			...
			case "command":
		    	...
		    	break;
		    default:
				result.put("status",miscUtil.utilLocal("api_not_exist")  );
    ...
    public String webDoc(Model model,@RequestParam Map<String, Object> params) {
    	...
    	{
	    	case "api":
	    		model.addAttribute("docName",miscUtil.utilLocal("api_list"));
	        	Login login=CacheUtil.threadlocallogin.get();
    ...
    public String webLLMSetting(Model model) {
    	utilBreadcrumb(model,new String[][]{
            {miscUtil.utilLocal("menu_llm"), "llmsetting"}
        });
        return "llmsetting"; 
    }
    ...
    {
    	Map<String,Object> result=aiService.webProcess(paramsBody);
    	return ResponseEntity.ok(result);
    }
    @GetMapping("/web/llmrulelog")
    public String webLLMRuleLog(@RequestParam Map<String, Object> paramsReq,Model model) {
    	return aiService.webRuleLog(paramsReq,model);
    }
    @GetMapping("/web/apisetting")
    public String webApiSetting(Model model) {
    	utilBreadcrumb(model,new String[][]{
            {miscUtil.utilLocal("menu_api"), "apisetting"}
        });
        return "apisetting"; 
    }
    ...
    public ResponseEntity<Map<String,Object>> webApiSettingRefresh(@RequestBody Map<String, Object> paramsBody) 
    {
    	...
    	if(action.equals("updateUrl")||action.equals("testUrl")||action.equals("updateEmail") ||action.equals("updateForward")||action.equals("updateSmtp")) 
    	{
			if(apiKeyModel==null)
    		{
    			result.put("status", miscUtil.utilLocal("create_key_first") );
    			return ResponseEntity.ok(result);
    		}
    	...
    	switch(action)
    	{
    		case "updateSmtp":
    	        ...
    	        if (smtpServer == null || smtpPort == null || smtpUserName == null || smtpPassword == null ||
        			...
        			result.put("status", miscUtil.utilLocal("error_smtp_enter_all"));
        			return ResponseEntity.ok(result);
    	        }
    	        if (!(smtpAuth == 0 || smtpAuth == 1)) {
        			result.put("status", miscUtil.utilLocal("error_smtp_auth"));
        			return ResponseEntity.ok(result);
    	        }
    	        if (!(smtpTls == 0 || smtpTls == 1)) {
        			result.put("status", miscUtil.utilLocal("error_smtp_tls"));
        			return ResponseEntity.ok(result);
    	        }
    	        if (!(smtpSsl == 0 || smtpSsl == 1)) {
        			result.put("status", miscUtil.utilLocal("error_smtp_ssl"));
        			return ResponseEntity.ok(result);
    	        }
    			...
    			result.put("info", miscUtil.utilLocal("update_ok"));
    			return ResponseEntity.ok(result);
    		case "keyAdd":
    		...
    		case "updateEmail":
    			...
    			if(forwardEmail.length()==0)//clear email address
    			{
    				...
    				{
            			result.put("status", miscUtil.utilLocal("email_already_empty") );
            			return ResponseEntity.ok(result);
    				}
        			...
        			mongoDBService.save("updateEmail", apiKeyModel);
        			result.put("alarmInfo", miscUtil.utilLocal("email_is_removed") );
        		...
        		}else if((((apiKeyModel.getForwardEmailStatus()<=1)||(apiKeyModel.getForwardEmail()==null)||(!apiKeyModel.getForwardEmail().equals(forwardEmail))))&&(forwardEmailVerificationCode.length()==0))
        		{
        			...
        			if((apiKeyModel.getForwardEmailVerificationTime()==null)||(MiscUtil.dateDiff(apiKeyModel.getForwardEmailVerificationTime(), now)>120))
        			{
	        			...
	        			mongoDBService.save("updateEmail", apiKeyModel);
	        			senderService.emailVerificationCode(forwardEmail,login.getUserModel(), apiKeyModel.getForwardEmailVerificationCode());
	        			result.put("alarmInfo", miscUtil.utilLocal("verification_sent_out") );
        			}else {
        				result.put("alarmInfo", miscUtil.utilLocal("verification_request_too_frequent") );
        			}
        		}else if(forwardEmailVerificationCode.length()>0)//verify email
        		{
        			...
        			{
            			result.put("alarmInfo", miscUtil.utilLocal("verification_code_expired") );
            			return ResponseEntity.ok(result);
        			}
        			if(!forwardEmailVerificationCode.equals(apiKeyModel.getForwardEmailVerificationCode()))
        			{
            			result.put("alarmInfo", miscUtil.utilLocal("verification_code_incorrect") );
            			return ResponseEntity.ok(result);
        			}
        			if(!forwardEmail.equals(apiKeyModel.getForwardEmail()))
        			{
            			result.put("alarmInfo", miscUtil.utilLocal("email_incorrect_with_before") );
            			return ResponseEntity.ok(result);
        			}
        			...
        			apiKeyModel.setForwardEmailVerificationCode(null);
        			mongoDBService.save("updateEmail", apiKeyModel);
        			result.put("alarmInfo", miscUtil.utilLocal("verification_successful") );
        		...
        		{
        			mongoDBService.save("updateEmail", apiKeyModel);
        			result.put("alarmInfo", miscUtil.utilLocal("rule_modified") );
        		}else
        			result.put("alarmInfo", miscUtil.utilLocal("nothing_changed") );
        		break;
    		case "updateForward":
    		...
    		case "updateUrl":
        		...
        		{
        			result.put("status", miscUtil.utilLocal("not_valid_url") );
        			return ResponseEntity.ok(result);
        		}
    		...
    		case "testUrl":
        		...
        		PostLogModel postLogModel=senderService.httpPostMap("xxxx","xxxx",apiKeyModel,ModuleDefine.MODULE_GENERAL,true);
        		...
        		result.put("info", miscUtil.utilLocal("test_success") + " "+checkStr);
    ...
    public String webUserList(Model model) {
    	utilBreadcrumb(model,new String[][]{
            {miscUtil.utilLocal("menu_users"), "userlist"}
        });
    ...
    private Object paramsUpdateValid(Map<String, Object[]> fieldsMeta, Map<String, Object> paramsBody,Map<String,Object> result) 
    {
    	...
    	{
    		result.put("status", miscUtil.utilLocal("field_name_wrong") );
    		return null;
    	}
    	String optionString="";
    	if(fieldType.equals("string"))
    	{
	        ...
	        if(fieldOption[0].toString().equals("list-string"))
	        {
	    		...
	    		{
	        		result.put("status", miscUtil.utilLocal("field_value_empty"));
	        		return null;
	    		}
	        	@SuppressWarnings("unchecked")
...
//    		}
//    	}
    	result.put("status", miscUtil.utilLocal("unknown_reason") );
        return null; 
    }
        ...
        if(users.trim().length()>0)
        {
	        ...
	        for (String id : checkedIds) {
	            ...
	            if((userId==null)||(userId.length()==0)) {
	            	result.put("status", id+" "+miscUtil.utilLocal("is_wrong"));
	                return null; 
	            }
    ...
    public ResponseEntity<Map<String,Object>> webUserListRefresh(@RequestBody Map<String, Object> paramsBody) {
    	...
    	switch(action)
    	{
	    	case "userPasswordReset":
	    		...
	    		{
	    			senderService.emailPasswordReset(userModelModified, password);
	    			result.put("alarmInfo",miscUtil.utilLocal("email_sent_inform_password_change"));
	    		}else {
	    			result.put("alarmInfo", miscUtil.utilLocal("password_changed_to") +" "+password);
	    		}
	    		break;
	    	case "userUpdate":
	    		...
	    		{
	    			result.put("alarmInfo",miscUtil.utilLocal("admin_only_operate"));
	    			return ResponseEntity.ok(result);
	    		}
    ...
    private boolean webUserListAdd(Map<String, Object> paramsBody,Map<String,Object> result)
    {
    	...
    	if(login==null)
    	{
    		result.put("status", miscUtil.utilLocal("not_login") );
    		return false;
    	}
    	...
    	{
    		result.put("status", miscUtil.utilLocal("username_password_empty") );
    		return false;
    	}
    	...
    	if(userModel!=null)
    	{
    		result.put("status", miscUtil.utilLocal("user_name_exist") );
    		return false;
    	}
    	...
    	senderService.emailUserCreate(userModel, password);
    	return true;
    }
    ...
    public String webUserSetting(Model model) {
    	utilBreadcrumb(model,new String[][]{
            {miscUtil.utilLocal("menu_settings"), "usersetting"}
        });
        return "usersetting"; 
    }
    ...
    public ResponseEntity<Map<String,Object>> webUserSettingRefresh(@RequestBody Map<String, Object> paramsBody) {
        ...
        if(login==null)
        {
        	result.put("status", miscUtil.utilLocal("no_access") );
        	return ResponseEntity.ok(result);        	
        }
        UserModel userModel=mongoDBService.findOneByField("id", login.getUserId(), UserModel.class);
        if(userModel==null)
        {
        	result.put("status", miscUtil.utilLocal("no_user") );
        	return ResponseEntity.ok(result);      
        }
        if(userModel.getUserStatus()==UserModel.UserStatus.DISABLED)
        {
    	...
    	switch(action)
    	{
	    	case "userUpdate":
	    		...
	    		if(fieldName.equals("Password"))
	    		{
	    	    	...
	    	    	result.put("alarmInfo", miscUtil.utilLocal("password_changed") );
	    		}else if(fieldName.equals("Login"))
	    		{
	    			...
	    			{
	    	        	result.put("status", miscUtil.utilLocal("same_as_current") );
	    	        	return ResponseEntity.ok(result);   
	    			}
	    			UserModel userModel2=mongoDBService.findOneByField("login", fieldValue, UserModel.class);
	    			if(userModel2!=null)
	    			{
	    	        	result.put("status", miscUtil.utilLocal("same_as_others")  );
	    	        	return ResponseEntity.ok(result);   
	    			}
	    			userModel.setLogin(fieldValue);
	    			userModel.setLoginChangeTime(now);
	    			mongoDBService.save("userupdate",userModel);
	    	    	result.put("alarmInfo", miscUtil.utilLocal("login_changed") );
	    		}else if(fieldName.equals("License"))
	    		{
	    			if((fieldValue!=null)&&(fieldValue.length()>10))
	    			{
	    				...
	    				if(importRet==null)
	    				{
	        	        	result.put("status", miscUtil.utilLocal("license_import_error") );
		    	        	return ResponseEntity.ok(result);   
	    				}//else
	    				//	result.put("alarmInfo", miscUtil.utilLocal("license_import_ok") );
	    			}
	    		}
	    		break;
    	}
    	List<Map<String,Object>> userInfo = new ArrayList<Map<String,Object>>();
    	MiscUtil.resultPutRow(userInfo, "name",miscUtil.utilLocal("user_level2"),"value",userModel.getUserType());	
		MiscUtil.resultPutRow(userInfo, "name",miscUtil.utilLocal("login2"),"value",userModel.getLogin());	
		MiscUtil.resultPutRow(userInfo, "name",miscUtil.utilLocal("login_change_time2"),"value",MiscUtil.dateFormat(userModel.getLoginChangeTime(),"MM-dd HH:mm:ss"));	
		MiscUtil.resultPutRow(userInfo, "name",miscUtil.utilLocal("password2"),"value","********");	
		MiscUtil.resultPutRow(userInfo, "name",miscUtil.utilLocal("password_change_time2"),"value",MiscUtil.dateFormat(userModel.getPassWordChangeTime(),"MM-dd HH:mm:ss"));	
		MiscUtil.resultPutRow(userInfo, "name",miscUtil.utilLocal("password_changed2"),"value",userModel.getPasswordType());	
		MiscUtil.resultPutRow(userInfo, "name",miscUtil.utilLocal("last_login_ip2"),"value",userModel.getLatestIp());	
		MiscUtil.resultPutRow(userInfo, "name",miscUtil.utilLocal("login_times2"),"value",userModel.getLoginTimes());	
		MiscUtil.resultPutRow(userInfo, "name",miscUtil.utilLocal("last_login_time2"),"value",MiscUtil.dateFormat(userModel.getLoginTime(),"MM-dd HH:mm:ss"));	
		MiscUtil.resultPutRow(userInfo, "name",miscUtil.utilLocal("login_failed_times2"),"value",userModel.getLoginFailedTimes());	
		MiscUtil.resultPutRow(userInfo, "name",miscUtil.utilLocal("last_login_failed_time2"),"value",MiscUtil.dateFormat(userModel.getLoginFailedTime(),"MM-dd HH:mm:ss"));	
		MiscUtil.resultPutRow(userInfo, "name",miscUtil.utilLocal("latest_visit_time2"),"value",MiscUtil.dateFormat(userModel.getLatestVisitTime(),"MM-dd HH:mm:ss"));	
		MiscUtil.resultPutRow(userInfo, "name",miscUtil.utilLocal("register_time2"),"value",MiscUtil.dateFormat(userModel.getRegisterTime(),"MM-dd HH:mm:ss"));	
		if(userModel.getUserType()==UserModel.UserType.ADMIN) {
			MiscUtil.resultPutRow(userInfo, "name",miscUtil.utilLocal("license_info"),"value",MiscUtil.strDisplay(commService.l()));	
			MiscUtil.resultPutRow(userInfo, "name",miscUtil.utilLocal("license_info2"),"value",miscUtil.utilLocal("license_info2_guide"));	 
		}
    ...
    private String webOTARsp(DeviceModel device,String ret) 
    {
    	...
    	if(otaRecord!=null)
    	{
			...
			download.setExpiredTime(MiscUtil.dateAddSeconds(new Date(), 10*60));
    ...
    public ResponseEntity<Map<String, Object>> webOTAUpload(@RequestParam MultipartFile file, @RequestParam Map<String, Object> paramsReq) {
        ...
        if (id == null) {
            result.put("status", miscUtil.utilLocal("wrong_id") );
            return ResponseEntity.ok(result);
        }
        if (file.isEmpty()) {
            result.put("status", miscUtil.utilLocal("file_is_empty") );
            return ResponseEntity.ok(result);
        }
        ...
        try (InputStream inputStream = file.getInputStream()) {
            ...
            {
                result.put("status", miscUtil.utilLocal("wrong_firmware") );
                return ResponseEntity.ok(result);
            }
            ...
            result.put("status", miscUtil.utilLocal("upload_successfully")  );
            return ResponseEntity.ok(result);
        } catch (Exception e) {
            result.put("status", miscUtil.utilLocal("upload_failed") );
            return ResponseEntity.ok(result);
        }
    }
    ...
    public String webOTAList(Model model) {
    	utilBreadcrumb(model,new String[][]{
            {miscUtil.utilLocal("menu_ota"), "otalist"}
        });
        return "otalist"; 
    }
    ...
    public ResponseEntity<Map<String,Object>> webOTAListRefresh(@RequestBody Map<String, Object> paramsBody) {
		...
		if(action.equals("otaDeviceList")||action.equals("otaDel")||action.equals("otaUpdate"))
		{
			...
			if(id.length()<5)
			{
				result.put("alarmInfo", miscUtil.utilLocal("id_empty")  );
				return ResponseEntity.ok(result);
			}
			...
			{
				result.put("alarmInfo", miscUtil.utilLocal("no_priv") );
				return ResponseEntity.ok(result);
			}
		}
    	switch(action)
    ...
    private boolean webOTAAddDeviceList(Map<String, Object> paramsBody,Map<String,Object> result,OTARecordModel recordModel)
    {
		...
		if(newList.size()==0)
		{
			if((tempList!=null)&&(tempList.size()>0))
				result.put("alarmInfo", miscUtil.utilLocal("no_valid_input_old_data_cleared") );
			else {
				result.put("alarmInfo", miscUtil.utilLocal("no_valid_input") );
				return false;
			}
		}else {
			if((tempList!=null)&&(tempList.size()>0))
				result.put("alarmInfo", miscUtil.utilLocal("replace_old_list_successfully") );
			else
				result.put("alarmInfo", miscUtil.utilLocal("add_device_list_successfully") );
		}
    ...
    private boolean webOTAListUpdate(Map<String, Object> paramsBody,Map<String,Object> result,Date now,OTARecordModel model)
    {
    	String fieldName = MiscUtil.webParamsGet(paramsBody, "fieldName", String.class, "");
    ...
    private OTARecordModel webOTAListAdd(String otaName,String comments,Map<String,Object> result)
    {
		...
		if(name.length()<2)
		{
			result.put("alarmInfo", miscUtil.utilLocal("at_least_characters_for_name") );
			return null;
		}
		...
		{
			result.put("alarmInfo", miscUtil.utilLocal("too_many_ota_records")  );
			return null;
		}
		Date now=new Date();
    ...
    private DataCacheModel dataCacheSave(String key,Object value,int expiredSeconds)
    {
		...
		dataCacheModel.setExpiredTime(MiscUtil.dateAddSeconds(now, expiredSeconds));
    ...
    public String webCodeList(Model model) {
      	utilBreadcrumb(model,new String[][]{
            {miscUtil.utilLocal("menu_project"), "codelist"}
        });
        return "webcodelist"; 
    }
    ...
    public ResponseEntity<Map<String,Object>> webCodeListRefresh(@RequestBody Map<String, Object> paramsBody) {
		...
		{
			result.put("alarmInfo",miscUtil.utilLocal("no_bind_device")); 
			return ResponseEntity.ok(result);
		}
    ...
    public ResponseEntity<Map<String,Object>> webCodeLogRefresh(@RequestBody Map<String, Object> paramsBody) {
    	...
    	switch(action)
    	{
    		case "saveBuild":
    			...
    			{
    				result.put("alarmInfo", miscUtil.utilLocal("not_successful_build") );
    				break;
    			}
	    		...
	    		if(binContent==null)
	    		{
	        		result.put("alarmInfo",  miscUtil.utilLocal("ota_bin_removed") );
	        		break;
	    		}
	    		String binMd5 = MiscUtil.MD5(binContent, null);
	    		String newFileId=mongoDBService.fileSave(null,"application/octet-stream", projectBuildModel.getBinFile(), binContent,"userId",userModel.getId());
	    		...
	    		result.put("alarmInfo",  miscUtil.utilLocal("ota_saved") );
    			break;
	    	case "delBuild":
	    		if((buildId==null)||(buildId.length()==0))
	    			break;
	    		...
	    		{
		    		result.put("alarmInfo", miscUtil.utilLocal("in_building") );
		    		break;
	    		}
    ...
    private boolean webCodeBuildSubmit(Map<String,Object> result,Date now,UserModel userModel,ProjectInfoModel projectInfoModel)
    {
    	boolean ret=false;
    	...
    	{
    		result.put("status", miscUtil.utilLocal("build_times_exceeds_quotation")+" "+userModel.getBuildProjectLimitPerDay());
    		return ret;
    	}
    	...
    	if(buildModel==null)
    	{
    		...
    		result.put("status", miscUtil.utilLocal("pending_build_exist") );
    	}
    	projectBuildService.buildProject();
    	return ret;
    }
    ...
    private boolean webCodeFileSave(Map<String,Object> result,Date now,UserModel userModel,ProjectInfoModel projectInfoModel,ProjectAccessModel projectAccessModel,String filePath,String fileContent)
    {
		if(fileContent.length()>10*1024*1024)
		{
	    	result.put("status", miscUtil.utilLocal("file_too_big") );
	    	return false;
		}
		if(!webCodeProjectFilePrivi(projectAccessModel,filePath,false))
		{
	    	result.put("status", miscUtil.utilLocal("no_access_file")  );
	    	return false;
		}
		Criteria queryCriteria = Criteria.where("userId").is(userModel.getId()).and("projectId").is(projectInfoModel.getId()).and("path").is(filePath);
		ProjectFileModel fileModel=mongoDBService.findOneByFields(queryCriteria, null,null,ProjectFileModel.class);
    ...
    private boolean webDeviceListAdd(Map<String, Object> paramsBody,Map<String,Object> result)
    {
    	...
    	{
    		result.put("status", miscUtil.utilLocal("lack_of_input") );
    		return false;
    	}
    	DeviceModel deviceModel=mongoDBService.findOneByField("deviceNo", deviceNo, DeviceModel.class);
    	if((deviceModel==null)||(deviceModel.getDeviceToken()==null)||(!deviceModel.getDeviceToken().equals(deviceToken)))
    	{
    		result.put("status", miscUtil.utilLocal("invalid_input") );
    		return false;
    	}
    	if((deviceModel.getDeviceTradeStatus()!=null)||(deviceModel.getDeviceTradeStatus()==DeviceModel.DeviceTradeStatus.CUSTOMER))
    	{
    		if(deviceModel.getUserId().equals(userId))
    			result.put("status", miscUtil.utilLocal("already_in_your_account") );
    		else
    			result.put("status", miscUtil.utilLocal("used_by_others") );
    		return false;
    	}
    	...
    	deviceModel.setModifyTime(new Date());
    ...
    public ResponseEntity<InputStreamResource> webCamera(@PathVariable String id,@RequestParam(value = "width", required = false) Integer width) {
        ...
        try {
            ...
            if(width==null || width==0)
            {
	            ...
	            BufferedImage originalImage = ImageIO.read(fsResource.getInputStream());
	            int thumbnailWidth = width; // 缩略图宽度
	            int thumbnailHeight = (int) (thumbnailWidth / ((double) originalImage.getWidth() / originalImage.getHeight())); // 维持原宽高比
	
	            BufferedImage thumbnail = new BufferedImage(thumbnailWidth, thumbnailHeight, originalImage.getType());
	            Graphics2D g2d = thumbnail.createGraphics();
	            g2d.drawImage(originalImage.getScaledInstance(thumbnailWidth, thumbnailHeight, Image.SCALE_SMOOTH), 0, 0, null);
	            g2d.dispose();
    ...
    @GetMapping("/web/modulecust")
    public String webModuleCust(@RequestParam Map<String, Object> paramsReq,Model model) {
    	...
    	if(moduleTypeId==-1 || deviceId.isEmpty())
    		return "notfound";
    	Object[] devicePriv=utilHasPriv(deviceId);
    	DeviceModel deviceModel=(DeviceModel)devicePriv[2];
    	if(deviceModel==null)
    		return "accessdeny";
    	ModuleType moduleType=ModuleDefine.ModuleType.fromId(moduleTypeId);
    	if(moduleType==null || moduleType.getHandler()==null)
    		return "accessdeny";
    	model.addAttribute("deviceId", deviceId);
    	model.addAttribute("deviceNo", deviceModel.getDeviceNo());
    	model.addAttribute("moduleTypeId", moduleTypeId);
    	return moduleType.getHandler().custPage(paramsReq, model);
    }
    @PostMapping("/web/modulecustrefresh")
    public ResponseEntity<Map<String,Object>> webModuleCustRefresh(@RequestBody Map<String, Object> paramsBody,HttpServletRequest request) {
    	Map<String,Object> result=new HashMap<String,Object>();
    	result.put("status", "ok");
    	String deviceId = MiscUtil.webParamsGet(paramsBody, "deviceId", String.class, "");
    	...
    	if(moduleType==null || moduleType.getHandler()==null)
    		return ResponseEntity.ok(result);
    	moduleType.getHandler().custPageRefresh(deviceModel,paramsBody, request,result);
    	return ResponseEntity.ok(result);
    }
    ...
    public String webDeviceModule(@RequestParam Map<String, Object> paramsReq,Model model) {
    	...
    	if(deviceModel==null)
    		return "notfound";
    	utilBreadcrumb(model,new String[][]{
            {miscUtil.utilLocal("menu_devices"), "deviceview?deviceId="+deviceId},
            {miscUtil.utilLocal("breadcrumb_module"), "deviceId="+deviceId+"&moduleTypeId="+moduleTypeId}
        });
    ...
    public ResponseEntity<Map<String,Object>> webDeviceModuleRefresh(@RequestBody Map<String, Object> paramsBody,HttpServletRequest request) {
    	...
    	String deviceId = MiscUtil.webParamsGet(paramsBody, "deviceId", String.class, "");
    	Integer moduleTypeId = MiscUtil.webParamsGet(paramsBody, "moduleTypeId", Integer.class, -1);
		Date startDate=MiscUtil.dateParse(MiscUtil.webParamsGet(paramsBody, "startDate", String.class, null),null);
		...
		if(moduleType==null)
		{
			result.put("status", miscUtil.utilLocal("no_module") +" "+moduleTypeId);
			return ResponseEntity.ok(result);
		}
		if((deviceModel.getModuleInfoModelMap()==null)||(!deviceModel.getModuleInfoModelMap().containsKey(moduleType.getId())))
		{
			result.put("status", miscUtil.utilLocal("no_module_for_device") +" "+deviceId);
			return ResponseEntity.ok(result);
		}
    	...
    	switch(action)
    	{
	    	case "configUpdate":
	    		...
	    		if(tempResult!=null)//error
	    			return tempResult;	    		
	    		moduleInfoModel.setModifyTime(new Date());
	    	...
	    	case "alertOk":
	    		...
	    		break;	
	    	case "infoRefresh":
	    		if(moduleHandler.getRefreshOperate()!=null && moduleHandler.getRefreshOperate().length()>0)
    	...
    	if(((fields==null)||(fields.contains("cust")))&&(moduleHandler.getSectionMap().containsKey("custContainer")))
    	{
	    	List<Map<String,Object>> custMapList=moduleHandler.custPageList(deviceModel);
	    	if(custMapList!=null)
	    		result.put("custList", custMapList);
    	}
    ...
    public String webDeviceSim(@RequestParam Map<String, Object> paramsReq,Model model) {
    	...
    	if(iccid!=null)
    	{
    		...
    		if(iccid.startsWith("89860"))
    		{
		    	...
		    	if(expired!=null) 
		    	{
		    		model.addAttribute("msg", miscUtil.utilLocal("sim_msg_normal"));
    		...
    		model.addAttribute("msg", miscUtil.utilLocal("sim_msg_no_card"));
        return "devicesim"; 
    }
    ...
    public String webDeviceCloudLog(@RequestParam Map<String, Object> paramsReq,Model model) {
    	...
    	Date startTime=MiscUtil.dateParse(MiscUtil.webParamsGet(paramsReq, "startDate", String.class, null),"yyyy-MM-dd HH:mm");
    	Date endTime=MiscUtil.dateParse(MiscUtil.webParamsGet(paramsReq, "endDate", String.class, null),"yyyy-MM-dd HH:mm");
    	if(deviceId.length()==0)
    		return "notfound";
    	Object[] devicePriv=utilHasPriv(deviceId);
    	if(((Boolean)devicePriv[0]==false)||(devicePriv[2]==null))
    	...
    	switch(action)
    	{
    		case "listLog":
    	    	...
    	    	{
    		    	if(startTime!=null) {
    		    		startTime=MiscUtil.dateTrimMinute(startTime);
    		    		conditionMap.put("startTime", startTime);
    		    	}
    		    	if(endTime!=null) {
    		    		endTime=MiscUtil.dateTrimMinute(endTime);
    		    		endTime=MiscUtil.dateAddMinutes(endTime, 1);
    	        ...
    	        List<DataCommModel> dataCommModelList=mongoDBService.find(conditionMap, sort, DataCommModel.class,0,1000,null);
    	        List<Map<String,String>> formattedLogs = new ArrayList<>();
    ...
    public ResponseEntity<Map<String,Object>> webDeviceViewRefresh(@RequestBody Map<String, Object> paramsBody) {
    	...
    	if(((Boolean)devicePriv[0]==false)||(devicePriv[2]==null))
    	{
			result.put("status", miscUtil.utilLocal("no_device") );
			return ResponseEntity.ok(result);
    	}
    	result.put("status", "ok");
    	DeviceModel deviceModel=(DeviceModel)devicePriv[2];
    	...
    	switch(action)
    	{
    		case "exeTime":
    			...
    			if(exeTime.length()>0) 
    			{
	    			List<Date[]> exeTimeList=aiService.webValidTime(exeTime);
	    			if(exeTimeList==null)
	    			{
	    			...
	    			deviceModel.setExeTime(exeTimeList);
	    			
    			}else
    				deviceModel.setExeTime(null);
    			mongoDBService.save("webDeviceViewRefresh1exeTime", deviceModel);
    			return ResponseEntity.ok(result);
    	...
    	if((fields==null)||(fields.contains("exetime"))) {
	    	if (deviceModel.getExeTime() != null) {
	            List<String> timePeriods = new ArrayList<>();
	            for (Date[] timeArray : deviceModel.getExeTime()) {
	            ...
	            result.put("exeTime", String.join(",", timePeriods));
	        }else
	        	result.put("exeTime", null);
    	}
	...
	private boolean webDeviceConfigDel(Map<String, Object> paramsBody,DeviceModel deviceModel,Map<String,Object> result) {
		...
		if(index.intValue()==0)
		{
			result.put("status",  miscUtil.utilLocal("can_not_del_version_config") );
			return false;
		}
		...
		result.put("status", miscUtil.utilLocal("del_config_failed") );
		return false;
	}
	...
	private ResponseEntity<Map<String, Object>> webDeviceConfigUpdate(Map<String, Object> paramsBody, Map<String, Object> result, DeviceModel deviceModel) {
		...
		if((configmap!=null)&&(configmap.containsKey(index)))
		{
			...
			if(element.getType()<=1)//number
			{
				if(fieldValue.length()>0)
				{
					...
					if(fieldValueLong==null) {
						result.put("status", miscUtil.utilLocal("value_in_wrong_format") );
						return ResponseEntity.ok(result);
					}
					long maxValue= 1L << (element.getValueLen()*8);
					if(fieldValueLong>maxValue) {
						result.put("status", miscUtil.utilLocal("value_too_big") );
						return ResponseEntity.ok(result);
					}else if(fieldValueLong<0)
					{
						result.put("status", miscUtil.utilLocal("value_is_negative") );
						return ResponseEntity.ok(result);
					}
					if((element.getType()==0)&&((element.getMinValue()>fieldValueLong)||(element.getMaxValue()<fieldValueLong)))//normal
					{
						result.put("status", miscUtil.utilLocal("between")+" "+element.getMinValue()+" "+miscUtil.utilLocal("and")+" "+element.getMaxValue());
						return ResponseEntity.ok(result);
					}
					if((element.getType()==1)&&(!element.getEnumValue().contains(fieldValueLong.intValue())))//enum
					{
						...
						result.put("status", miscUtil.utilLocal("should_be")+" "+errMsg);
						return ResponseEntity.ok(result);
					}
				}
			...
			}else if(element.getType()<=54)//string
			{
				if(fieldValue.length()>element.getValueLen()) {
					result.put("status", miscUtil.utilLocal("string_too_long") );
					return ResponseEntity.ok(result);
				}
				if((fieldValue.length()>element.getMaxValue())||(fieldValue.length()<element.getMinValue()))
				{
					result.put("status", miscUtil.utilLocal("string_length_between")+" "+element.getMinValue()+" "+miscUtil.utilLocal("and")+" "+element.getMaxValue());
					return ResponseEntity.ok(result);
				}
				if((element.getType()==52)&&(!MiscUtil.ipV4Valid(fieldValue)))//ip v4
				{
					result.put("status", miscUtil.utilLocal("not_valid_ip_address") );
					return ResponseEntity.ok(result);
				}
		...
		{
			result.put("status", miscUtil.utilLocal("no_this_index")  );
			return ResponseEntity.ok(result);
		}
		return null;
	}
...
//    }
    ...
    public Map<String,Object> test() {
    	...
    	commModel.setDeviceNo(deviceModel.getDeviceNo());
    	commModel.setUploadTime(new Date());
    	mongoDBService.save("test", commModel);
    	...
    	user.setPassword("x2");
    	user.setUserId("x3");
    	mongoDBService.save("test", user);
    	//fileSave(Integer expireSeconds, String contentType, String filename, byte[] data, Object... params)
